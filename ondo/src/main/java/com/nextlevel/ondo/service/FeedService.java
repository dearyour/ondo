        ...
        for (Feed f : list) {
            ...
            for(Comment c : comments){
                if(c.getUser().getUserId() == tokenuser.getUserId()){
                    DetailCommentDto detailCommentDto = new DetailCommentDto(c,c.getUser().getUsername(),false);
                    detailCommentDtos.add(detailCommentDto);
                } else {
                    DetailCommentDto detailCommentDto = new DetailCommentDto(c,c.getUser().getUsername(),true); //토큰아이디랑 작성아이디 다르면 활성화 true
                    detailCommentDtos.add(detailCommentDto);
                }
            }
            Boolean flag = true;
...
//        }
    ...
    public DetailFeedDto detailFeed(long feedId, String token) {
        ...
        for (Comment c : comments) {
            if (c.getUser().getUserId() == tokenuser.getUserId()) {
                DetailCommentDto detailCommentDto = new DetailCommentDto(c,c.getUser().getUsername(), true);
                detailCommentDtos.add(detailCommentDto);
            } else {
                DetailCommentDto detailCommentDto = new DetailCommentDto(c,c.getUser().getUsername(), false);
                detailCommentDtos.add(detailCommentDto);
