...
public class S3Uploader {
    ...
    public String upload(MultipartFile multipartFile, String dirName, String target) throws IOException {
        System.out.println(multipartFile);
        File uploadFile = convert(multipartFile)  // 파일 변환할 수 없으면 에러
                .orElseThrow(() -> new IllegalArgumentException("error: MultipartFile -> File convert fail"));

        return upload(uploadFile, dirName + "/" + target, target);
    }

    // S3로 파일 업로드하기
    private String upload(File uploadFile, String dirName, String target) {
...
//        }
    ...
    private Optional<File> convert(MultipartFile file) throws IOException {
        ...
        Runtime.getRuntime().exec("sudo chmod 777 " + name);
