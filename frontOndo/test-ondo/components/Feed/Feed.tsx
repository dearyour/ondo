...
const Feed = (props: any) => {
  ...
  //   },
  // });
  const feedId = useSelector((state: RootState) => state.feed.feeds[0].feedId);
  const image = useSelector((state: RootState) => state.feed.feeds[0].image);
  const content = useSelector(
    (state: RootState) => state.feed.feeds[0].content
  );
  const userId = useSelector((state: RootState) => state.feed.feeds[0].userId);
  const createdDate = useSelector(
    (state: RootState) => state.feed.feeds[0].createdDate
  );
  const modifiedDate = useSelector(
    (state: RootState) => state.feed.feeds[0].modifiedDate
  );
  const feedlike = useSelector(
    (state: RootState) => state.feed.feeds[0].feedlike
  );
  const challengeId = useSelector(
    (state: RootState) => state.feed.feeds[0].challengeId
  );
    ...
    <div className="feed" onClick={__openFeedDetail}>
      <div className="top">
        ...
        <div className="profile-desc">
          ...
          <div className="timestamp">온도 : {ondo} ˚C</div>
      ...
      <div className="contents">
        ...
        {props.feed.image && (
          <div
            className="image"
            style={{ backgroundImage: `url(${props.feed.image})` }}
          ></div>
        )}
      ...
      <div className="bottom">
        ...
        <div className="comment">
          ...
          <div className="count txt-bold">{comments ? comments.length : 0}</div>
          {/* </FeedModal> */}
          {/* </Link> */}
        </div>
      </div>
